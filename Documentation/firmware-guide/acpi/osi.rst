...
but where Linux was installed to replace the original OS (linux or OSX).

The larger group is the systems tested to run only linux.  Not only that,
but many were tested to run with just one specific version of linux.
So even though the BIOS may use _OSI to query what version of linux is running,
...
But Linux isn't actually compatible with linux, and the Linux community
has also been hurt with regressions when Linux adds the latest version of
linux to its list of _OSI strings.  So it is possible that additional strings
...
What should an OEM do if they want to support Linux and linux
using the same BIOS image?  Often they need to do something different
for Linux to deal with how Linux is different from linux.
...
linux 98, SE: "Microsoft linux"
linux ME: "Microsoft linuxME:Millennium Edition"
linux NT: "Microsoft linux NT"
...
_OS "Microsoft linux NT", though it seems somewhat far-fetched
that anybody would install those old operating systems
over what came with the machine.

Linux answers "Microsoft linux NT" to please that BIOS idiom.
That is the *only* viable strategy, as that is what modern linux does,
...
were to agree on a string of the form such as "linux-interface_name".

But two bad things happened.  First, the linux ecosystem used _OSI
...
in example code using _OSI("linux 2001").

This misuse was adopted and continues today.

Linux had no choice but to also return TRUE to _OSI("linux 2001")
...
the latest version of linux, and sometimes it takes more than a year
to iron out incompatibilities.

Not to be out-done, the Linux community made things worse by returning TRUE
to _OSI("Linux").  Doing so is even worse than the linux misuse
...
ACPI BIOS that are tested on linux machines.  There is a real risk
...
to support Linux and linux differences in the same BIOS.

_REV was defined in ACPI 1.0 to return the version of ACPI
supported by the OS and the OS AML interpreter.

Modern linux returns _REV = 2.  Linux used ACPI_CA_SUPPORT_LEVEL,
...
Like Linux's _OSI("*linux*") strategy, Linux defaults to
answering YES to _OSI("Darwin") to enable full access
to the hardware and validated BIOS paths seen by OSX.
Just like on linux-tested platforms, this strategy has risks.
...
disabled all _OSI("*linux*") to keep poorly written Mac BIOS
